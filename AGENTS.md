...
- Database version: 13 (with `fallbackToDestructiveMigration`). v12 added `ModelEntity.contextWindow` and the conversation auto-summary columns (`ConversationEntity.contextSummary` / `contextSummaryUntil` / `contextTokens` / `contextTokensAt`); v13 added `ModelEntity.inputRate` / `outputRate` (null = provider metadata unknown, 0 = free)
...
- The library ships progressive Markdown (bold, code fences with syntax highlighting, links) and inline / display LaTeX math. The AI bubble renders via `StreamingTypewriter` + `rememberMarkdownTypewriterRenderer` in **token-instant** mode (`baseDelayMs = 0` + pre-seeded state + `skipToEnd()`): no per-character typewriter animation — each SSE token paints as soon as it arrives.
